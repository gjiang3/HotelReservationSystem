...
public class Model {
	...
	public ArrayList<Account> getAllUsers(){
		...
		try {
			...
			while (rs.next()){
				users.add(new Account(rs.getString("firstName"), rs.getString("lastName"), rs.getString("username"), 
						rs.getString("userRole"), rs.getInt("age"), rs.getString("gender")));
			}
	...
	public User getAccountInfo(String username){
		User user = null;
		String queryAccount = "SELECT * FROM USER WHERE username = '" + username + "'"; 
		try {
			ResultSet rs = statement.executeQuery(queryAccount);
			if (rs.next()) {
				user = new User(rs.getString("firstname"), rs.getString("lastname"), 
						rs.getString("username"), rs.getString("userrole"), rs.getInt("age"), rs.getString("gender"),
						rs.getString("password"), rs.getString("question"), rs.getString("answer"));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public Account getAccount(String username) {
		Account acc = null;
		String queryAccount = "SELECT firstname, lastname, username, userrole, gender, age  "
	...
	public boolean changeAccountInfo(String pass, String first, String last, int age, String gen, String secQuestion, String secAnswer){
		String query = "Update USER set password = '" + pass + "', firstName = '" + first + "', lastName = '" + last + "', age = " + age + "," 
				 + "question = '" + secQuestion + "', answer = '" + secAnswer + "' where username = '" + currentUser.getUsername() + "'" ;
		try {
			statement.executeUpdate(query);
			return true;
		} catch (SQLException e) {
	...
	public String getStatistics() {
		...
		try {
			int totalUer = 0;
			ResultSet rs = statement.executeQuery(totalUser);
			if (rs.next()) 
				totalUer = Integer.parseInt(rs.getString(1));
			rs.close();
			
			rs = statement.executeQuery(totalCustomer);
			int numCustomer = 0;
			int numEmployee = 0;
			if (rs.next()){
				numCustomer = Integer.parseInt(rs.getString(1));
				numEmployee = totalUer - numCustomer;
			}
			rs.close();
			
			rs = statement.executeQuery(totalManager);
			int numManager = 0;
			int numRoomAttendant = 0;
			if (rs.next()){
				numManager = Integer.parseInt(rs.getString(1));
				numRoomAttendant = numEmployee - numManager;
			}

			result += "Total employee: " + numEmployee;
			result += "\nNumber of manager: " + numManager;
			result += "\nNumber of room attendant: " + numRoomAttendant;
			result += "\nNumber of registered customer: " + numCustomer;	
			rs = statement.executeQuery(avgAge);
			if (rs.next()){
				String avg = "0";
			...
			rs = statement.executeQuery(avgRating);
			if (rs.next()){
				String avg = "0";
				if(rs.getString(1) != null){
					avg = String.format( "%.2f", Double.parseDouble(rs.getString(1)) );
	...
	public boolean addRoomService(String username, String task) {	
		String query = String.format("INSERT INTO ROOMSERVICE(username, task)"
	...
	public ArrayList<RoomService> getRoomService() {
		String query = "select * from roomservice where completedBy is NULL";
		ArrayList<RoomService> roomservice = new ArrayList<RoomService>();
		try {
			ResultSet rs = statement.executeQuery(query);
			while (rs.next()) {
					roomservice.add(new RoomService(rs.getInt("taskId"), rs.getString("task"), rs.getString("username")));
			}
			return roomservice;
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public void updateRating(String username, int rate){
		String query = "update rating set rating =  " + rate +" where customer = '" + username + "'";
		try {
			statement.executeUpdate(query);
